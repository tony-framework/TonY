...
public class THSContainer {

  private static final Log LOG = LogFactory.getLog(THSContainer.class);

  private THSConfiguration conf;
  ...
  private THSContainer() {
    this.conf = new THSConfiguration();
    conf.addResource(new Path(THSConstants.THS_JOB_CONFIGURATION));
    LOG.info("user is " + conf.get("hadoop.job.ugi"));
    this.containerId = new THSContainerId(ConverterUtils.toContainerId(System
        .getenv(ApplicationConstants.Environment.CONTAINER_ID.name())));
    this.downloadRetry = conf.getInt(THSConfiguration.THS_DOWNLOAD_FILE_RETRY, THSConfiguration.DEFAULT_THS_DOWNLOAD_FILE_RETRY);
    this.envs = System.getenv();
    this.xlearningAppType = envs.get(THSConstants.Environment.THS_APP_TYPE.toString()).toUpperCase();
    this.role = envs.get(THSConstants.Environment.THS_TF_ROLE.toString());
    ...
    this.index = Integer.valueOf(envs.get(THSConstants.Environment.THS_TF_INDEX.toString()));
    if ("TENSORFLOW".equals(xlearningAppType)) {
    ...
    this.single = conf.getBoolean(THSConfiguration.THS_MODE_SINGLE, THSConfiguration.DEFAULT_THS_MODE_SINGLE);
    heartbeatInterval = this.conf.getInt(THSConfiguration.THS_CONTAINER_HEARTBEAT_INTERVAL, THSConfiguration.DEFAULT_THS_CONTAINER_HEARTBEAT_INTERVAL);
    reservedSocket = new Socket();
  }

  private void init() {
    LOG.info("THSContainer initializing");
    String appMasterHost = System.getenv(THSConstants.Environment.APPMASTER_HOST.toString());
    int appMasterPort = Integer.valueOf(System.getenv(THSConstants.Environment.APPMASTER_PORT.toString()));
    ...
    heartbeatThread.setContainerStatus(THSContainerStatus.INITIALIZING);
  ...
  private THSContainerId getContainerId() {
  ...
  private void prepareInputFiles() throws IOException, InterruptedException,
      ExecutionException {
    if (conf.get(THSConfiguration.THS_INPUT_STRATEGY, THSConfiguration.DEFAULT_THS_INPUT_STRATEGY).toUpperCase().equals("STREAM")) {
      LOG.info("THS_INPUT_STRATEGY is STREAM, use the stream way to read data from hdfs.");
    } else if (conf.get(THSConfiguration.THS_INPUT_STRATEGY, THSConfiguration.DEFAULT_THS_INPUT_STRATEGY).toUpperCase().equals("PLACEHOLDER")) {
      ...
      ExecutorService executor = Executors.newFixedThreadPool(
          conf.getInt(THSConfiguration.THS_DOWNLOAD_FILE_THREAD_NUMS, THSConfiguration.DEFAULT_THS_DOWNLOAD_FILE_THREAD_NUMS),
      ...
      for (InputInfo inputInfo : inputs) {
        ...
        for (Path path : inputInfo.getPaths()) {
          String downloadDst;
          if (conf.getBoolean(THSConfiguration.THS_INPUTFILE_RENAME, THSConfiguration.DEFAULT_THS_INPUTFILE_RENAME)) {
  ...
  private void createLocalOutputDir() {
    if (this.conf.get(THSConfiguration.THS_OUTPUT_STRATEGY, THSConfiguration.DEFAULT_THS_OUTPUT_STRATEGY).toUpperCase().equals("STREAM")) {
      LOG.info("THS_OUTPUT_STRATEGY is STREAM, do not need to create local output dir.");
    } else {
    ...
    int boardIndex = this.conf.getInt(THSConfiguration.THS_TF_BOARD_WORKER_INDEX, THSConfiguration.DEFAULT_THS_TF_BOARD_WORKER_INDEX);
    Boolean boardEnable = this.conf.getBoolean(THSConfiguration.THS_TF_BOARD_ENABLE, THSConfiguration.DEFAULT_THS_TF_BOARD_ENABLE);
    if (boardEnable && this.role.equals(THSConstants.WORKER) && boardIndex == this.index) {
      if (this.conf.get(THSConfiguration.THS_TF_BOARD_LOG_DIR, THSConfiguration.DEFAULT_THS_TF_BOARD_LOG_DIR).indexOf("hdfs://") == -1) {
        Utilities.mkdirs(this.conf.get(THSConfiguration.THS_TF_BOARD_LOG_DIR, THSConfiguration.DEFAULT_THS_TF_BOARD_LOG_DIR));
        LOG.info("Created board log dir " + this.conf.get(THSConfiguration.THS_TF_BOARD_LOG_DIR, THSConfiguration.DEFAULT_THS_TF_BOARD_LOG_DIR));
      } else {
        LOG.info("User appoint the board log dir : " + this.conf.get(THSConfiguration.THS_TF_BOARD_LOG_DIR));
      }
    }
  }
  ...
  private void uploadOutputFiles() throws IOException {
    if (this.conf.get(THSConfiguration.THS_OUTPUT_STRATEGY, THSConfiguration.DEFAULT_THS_OUTPUT_STRATEGY).toUpperCase().equals("STREAM")) {
      LOG.info("THS_OUTPUT_STRATEGY is STREAM, do not need to upload local output files.");
    } else {
      List<OutputInfo> outputs = Arrays.asList(amClient.getOutputLocation());
    ...
    if (this.conf.get(THSConfiguration.THS_TF_BOARD_LOG_DIR, THSConfiguration.DEFAULT_THS_TF_BOARD_LOG_DIR).indexOf("hdfs://") == -1) {
      THSConfiguration xlConf = new THSConfiguration();
      int boardIndex = conf.getInt(THSConfiguration.THS_TF_BOARD_WORKER_INDEX, THSConfiguration.DEFAULT_THS_TF_BOARD_WORKER_INDEX);
      Boolean boardEnable = conf.getBoolean(THSConfiguration.THS_TF_BOARD_ENABLE, THSConfiguration.DEFAULT_THS_TF_BOARD_ENABLE);
      String boardLogDir = conf.get(THSConfiguration.THS_TF_BOARD_LOG_DIR, THSConfiguration.DEFAULT_THS_TF_BOARD_LOG_DIR);
      Path localLogPath = new Path(boardLogDir);
      FileSystem boardLocalFs = FileSystem.getLocal(conf);
      String boardHistoryDir;
      Path remoteLogPath;
      FileSystem boardDfs;
      if (conf.get(THSConfiguration.THS_TF_BOARD_HISTORY_DIR, THSConfiguration.DEFAULT_THS_TF_BOARD_HISTORY_DIR).equals(xlConf.get(
          THSConfiguration.THS_TF_BOARD_HISTORY_DIR, THSConfiguration.DEFAULT_THS_TF_BOARD_HISTORY_DIR))) {
        boardHistoryDir = xlConf.get("fs.defaultFS") + conf.get(THSConfiguration.THS_TF_BOARD_HISTORY_DIR,
            THSConfiguration.DEFAULT_THS_TF_BOARD_HISTORY_DIR) + "/" + this.envs.get("APP_ID");
        remoteLogPath = new Path(boardHistoryDir);
        boardDfs = remoteLogPath.getFileSystem(xlConf);
      } else {
        boardHistoryDir = conf.get("fs.defaultFS") + conf.get(THSConfiguration.THS_TF_BOARD_HISTORY_DIR,
  ...
  private Boolean run() throws IOException {
    try {
      if (this.role.equals(THSConstants.WORKER)) {
        prepareInputFiles();
      }
      if (this.conf.getBoolean(THSConfiguration.THS_CONTAINER_AUTO_CREATE_OUTPUT_DIR, THSConfiguration.DEFAULT_THS_CONTAINER_AUTO_CREATE_OUTPUT_DIR)) {
    ...
    if (xlearningAppType.equals("LIGHTLDA") && !single) {
      if (this.role.equals(THSConstants.PS)) {
      ...
      if (this.role.equals(THSConstants.WORKER)) {
        ...
        while (!heartbeatThread.isXLearningTrainCompleted()) {
          ...
          Utilities.sleep(this.conf.getInt(THSConfiguration.THS_CONTAINER_UPDATE_APP_STATUS_INTERVAL, THSConfiguration.DEFAULT_THS_CONTAINER_UPDATE_APP_STATUS_INTERVAL));
        }
    ...
    envList.add(THSConstants.Environment.THS_INPUT_FILE_LIST.toString() + "=" + this.inputFileList);

    if ("TENSORFLOW".equals(xlearningAppType)) {
      envList.add(THSConstants.Environment.THS_TF_INDEX.toString() + "=" + this.index);
      envList.add(THSConstants.Environment.THS_TF_ROLE.toString() + "=" + this.role);
      if (!single) {
        /**
         * set TF_CLUSTER_DEF in env
         * python script can load cluster def use "json.loads(os.environ["CLUSTER_DEF"])"
         */
        envList.add(THSConstants.Environment.THS_TF_CLUSTER_DEF.toString() + "=" + this.clusterDef);
      }
      ...
      envList.add("LIGHTGBM_NUM_MACHINE=" + System.getenv(THSConstants.Environment.THS_LIGHTGBM_WORKER_NUM.toString()));
      ...
      if (!single) {
        envList.add("LIGHTLDA_WORKER_NUM=" + System.getenv(THSConstants.Environment.THS_LIGHTLDA_WORKER_NUM.toString()));
        envList.add("LIGHTLDA_SERVER_NUM=" + System.getenv(THSConstants.Environment.THS_LIGHTLDA_PS_NUM.toString()));
        envList.add("LIGHTLDA_SERVER_ENDPOINT=" + this.lightLDAEndpoint);
      }
    }

    if (conf.get(THSConfiguration.THS_INPUT_STRATEGY, THSConfiguration.DEFAULT_THS_INPUT_STRATEGY).toUpperCase().equals("PLACEHOLDER")) {
      envList.add(THSConstants.Environment.THS_INPUT_FILE_LIST.toString() + "=" + this.inputFileList);
      if (envList.toString().length() > conf.getInt(THSConfiguration.THS_ENV_MAXLENGTH, THSConfiguration.DEFAULT_THS_ENV_MAXLENGTH)) {
        LOG.warn("Current container environments length " + envList.toString().length() + " exceed the configuration " + THSConfiguration.THS_ENV_MAXLENGTH + " " + conf.getInt(
            THSConfiguration.THS_ENV_MAXLENGTH, THSConfiguration.DEFAULT_THS_ENV_MAXLENGTH));
    ...
    String command = envs.get(THSConstants.Environment.THS_EXEC_CMD.toString());
      ...
      Thread stdinRedirectThread = new Thread(new Runnable() {
        ...
        public void run() {
          try {
            ...
            File gzFile = new File(conf.get(THSConfiguration.THS_INPUTFORMAT_CACHEFILE_NAME, THSConfiguration.DEFAULT_THS_INPUTFORMAT_CACHEFILE_NAME));
            GZIPOutputStream gos = new GZIPOutputStream(new FileOutputStream(gzFile));
            boolean isCache = conf.getBoolean(THSConfiguration.THS_INPUTFORMAT_CACHE, THSConfiguration.DEFAULT_THS_INPUTFORMAT_CACHE);
            List<InputSplit> inputs = Arrays.asList(amClient.getStreamInputSplit(containerId));
            JobConf jobConf = new JobConf(conf);
            RecordReader reader;
            InputFormat inputFormat = ReflectionUtils.newInstance(conf.getClass(THSConfiguration.THS_INPUTF0RMAT_CLASS, THSConfiguration.DEFAULT_THS_INPUTF0RMAT_CLASS, InputFormat.class),
                jobConf);
            for (int j = 0; j < conf.getInt(THSConfiguration.THS_STREAM_EPOCH, THSConfiguration.DEFAULT_THS_STREAM_EPOCH); j++) {
              ...
              for (int i = 0, len = inputs.size(); i < len; i++) {
                ...
                while (!finished) {
                  try {
                    ...
                    osw.write("\n");
                    if (j == 0 && isCache) {
                      if (conf.getInt(THSConfiguration.THS_STREAM_EPOCH, THSConfiguration.DEFAULT_THS_STREAM_EPOCH) > 1) {
                        gos.write(value.toString().getBytes());
                        gos.write("\n".getBytes());

                        if ((gzFile.length() / 1024 / 1024) > conf.getInt(THSConfiguration.THS_INPUTFORMAT_CACHESIZE_LIMIT, THSConfiguration.DEFAULT_THS_INPUTFORMAT_CACHESIZE_LIMIT)) {
                          LOG.info("Inputformat cache file size is:" + gzFile.length() / 1024 / 1024 + "M "
                              + "beyond the limit size:" + conf.getInt(THSConfiguration.THS_INPUTFORMAT_CACHESIZE_LIMIT, THSConfiguration.DEFAULT_THS_INPUTFORMAT_CACHESIZE_LIMIT) + "M.");
                          gzFile.delete();
    ...
    List<OutputInfo> outputs = Arrays.asList(amClient.getOutputLocation());
    if ((this.conf.get(THSConfiguration.THS_OUTPUT_STRATEGY, THSConfiguration.DEFAULT_THS_OUTPUT_STRATEGY).equals("STREAM")) && outputs.size() > 0) {
      ...
      final Thread stdoutRedirectThread = new Thread(new Runnable() {
        ...
        public void run() {
          try {
            ...
            jobConf.set(THSConstants.STREAM_OUTPUT_DIR, remotePath.makeQualified(dfs).toString());
            OutputFormat outputFormat = ReflectionUtils.newInstance(conf.getClass(THSConfiguration.THS_OUTPUTFORMAT_CLASS, THSConfiguration.DEFAULT_THS_OUTPUTF0RMAT_CLASS, OutputFormat.class),
                jobConf);
    ...
    heartbeatThread.setContainerStatus(THSContainerStatus.RUNNING);
    ...
    if (boardEnable && this.role.equals(THSConstants.WORKER) && boardIndex == this.index) {
      ...
      String boardLogDir = this.conf.get(THSConfiguration.THS_TF_BOARD_LOG_DIR, THSConfiguration.DEFAULT_THS_TF_BOARD_LOG_DIR);
      int boardPort = boardReservedSocket.getLocalPort();
      String boardCommand;
      if ("TENSORFLOW".equals(xlearningAppType)) {
        int boardReloadInterval = this.conf.getInt(THSConfiguration.THS_TF_BOARD_RELOAD_INTERVAL, THSConfiguration.DEFAULT_THS_TF_BOARD_RELOAD_INTERVAL);
        boardCommand = this.conf.get(THSConfiguration.THS_TF_BOARD_PATH, THSConfiguration.DEFAULT_THS_TF_BOARD_PATH) + " --host=" + boardHost + " --port=" + boardPort + " --reload_interval=" + boardReloadInterval + " --logdir=" + boardLogDir;
      } else {
        int boardCacheTimeout = this.conf.getInt(THSConfiguration.THS_BOARD_CACHE_TIMEOUT, THSConfiguration.DEFAULT_THS_BOARD_CACHE_TIMEOUT);
        boardCommand = this.conf.get(THSConfiguration.THS_BOARD_PATH, THSConfiguration.DEFAULT_THS_BOARD_PATH) + " --host=" + boardHost + " --port=" + boardPort + " --logdir=" + boardLogDir + " --cache_timeout=" + boardCacheTimeout;
        String modelpb = this.conf.get(THSConfiguration.THS_BOARD_MODELPB, THSConfiguration.DEFAULT_THS_BOARD_MODELPB);
    ...
    int updateAppStatusInterval = this.conf.getInt(THSConfiguration.THS_CONTAINER_UPDATE_APP_STATUS_INTERVAL, THSConfiguration.DEFAULT_THS_CONTAINER_UPDATE_APP_STATUS_INTERVAL);
    ...
    if (this.role.equals(THSConstants.PS) && this.xlearningAppType.equals("TENSORFLOW")) {
      if (code == -1 || code == 0) {
        this.uploadOutputFiles();
      }
    }

    if (this.role.equals(THSConstants.PS) && !this.xlearningAppType.equals("LIGHTLDA")) {
      if (code == -1) {
  ...
  private void reportFailedAndExit() {
    ...
    heartbeatThread.setContainerStatus(THSContainerStatus.FAILED);
  ...
  private void reportSucceededAndExit() {
    Date now = new Date();
    heartbeatThread.setContainersFinishTime(now.toString());
    heartbeatThread.setContainerStatus(THSContainerStatus.SUCCEEDED);
    Utilities.sleep(heartbeatInterval);
    System.exit(0);
  }

  public static void main(String[] args) {
    THSContainer container = new THSContainer();
    try {
      container.init();
      if (container.run()) {
        LOG.info("THSContainer " + container.getContainerId().toString() + " finish successfully");
        container.reportSucceededAndExit();
      } else {
        LOG.error("THSContainer run failed!");
